...
def _client(storage_dir: str | None = None) -> DocuNav:
    return DocuNav(storage_dir=storage_dir)
...
def cli(ctx: click.Context, storage_dir: str | None) -> None:
    """DocuNav — Agent-native document intelligence layer."""
...
@cli.command()
def serve() -> None:
    """Start the DocuNav API server."""
    import uvicorn
    uvicorn.run("docunav.api.app:app", host="0.0.0.0", port=8000, reload=True)
